#!/usr/bin/env node
/**
 * Veil Protocol — quick demo runner.
 *
 ...
 *
 * Usage: node scripts/demo-veil.mjs
 *    or: npm run demo:veil
 */
...
const banner = `
═══════════════════════════════════════════════════
  Veil Protocol — stealth × x402 demo
═══════════════════════════════════════════════════
`;
...
async function serverReady() {
  try {
    ...
    return !!(process.env.VEIL_SKIP_SERVER_CHECK);
  }
}
...
if (!ok) {
  ...
  console.error("Or set AGENT_SERVER_URL / VEIL_SKIP_SERVER_CHECK=1 to bypass this check.");
  process.exit(1);
}
